%%%%%%%%%%%%%      EXPANDED FLOW WIH SHOCK            %%%%%%%%%%%%%%

function [tvec_realtime, rho2, p2, T2, v2, Ma2] = ...
    NozzleConservativeWithShockFun(tfin, x, A, pres, pext, Tres, Text0, gamma, mw_air, R)
...
pright = pext/pref; %Dimensionless pressure
...
p0 = ((pres-pext)*exp(-x/(L/5)) + pext)/pref;
T0 = ((Tres-Text0)*exp(-x/(L/5)) + Text0)/Tref;
v0 = 1e-3*ones(size(x));
rho0 = p0./T0;
e0 = T0;
...
while tval < tfin
     ...
     %With Shock
     vN = U2(Nx)/U1(Nx); 
     U3(Nx) = pright*A(Nx)/(gamma-1) + (gamma/2)*U2(Nx)*vN;
