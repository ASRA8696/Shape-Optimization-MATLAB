%% Main script for NozzleConservative simulation - Nozzle comparison
clear
close
clc
...
Nx = 301;
x = linspace(0,L,Nx);
tfin = 10;     %Dimensionless time. 
               % Represents number of times sound waves can travel length L of Nozzle
...
A1fun = @(x) 1 + 3*(x - 1.5).^2; % 0 < x < 1 [1]
A1 = A1fun(x);

A2fun = @(x) 1 + 3*(x - 1).^2; % 0 < x < 1 [1]
A2 = A2fun(x);
...
[tvec_realtime1, rho1, p1, T1, v1, Ma1] = ...
    NozzleConservativeWithShockFun(tfin, x, A1, pres, pext, Tres, Text0, ...
                                    gamma, mw_air, R);
                                
[tvec_realtime2, rho2, p2, T2, v2, Ma2] = ...
    NozzleConservativeWithShockFun(tfin, x, A2, pres, pext, Tres, Text0, ...
                                    gamma, mw_air, R);  
                                                             
[tvec_realtime3, rho3, p3, T3, v3, Ma3] = ...
    NozzleConservativeWithShockFun(tfin, x, A3, pres, pext, Tres, Text0, ...
                                    gamma, mw_air, R); 
